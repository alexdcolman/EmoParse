...
def caracterizar_variable(df_emociones, df_recortes, modelo_llm, prompt_base, heuristicas_path, ontologia_path,
                          mostrar_prompt=False, path_errores=None, output_path=None, guardar=True,
                          checkpoint_interval=10):
    import os

    contador = 0  # contador global para checkpoints

    for idx in tqdm(range(len(df_emociones))):
        df_out, data = procesar_una_emocion(
        ...
        if not df_out.empty and guardar and output_path:
            # Guardado inmediato, header solo la primera vez
            df_out.to_csv(
                output_path,
                index=False,
                mode='a',
    ...
    return pd.DataFrame()  # opcional, se puede ignorar
...
def caracterizar_foria(df_emociones, df_recortes, modelo_llm, mostrar_prompt=False, path_errores=None):
    ...
    return caracterizar_variable(df_emociones, df_recortes, modelo_llm, PROMPT_FORIA, heuristicas_path, ontologia_path,
...
def caracterizar_dominancia(df_emociones, df_recortes, modelo_llm, mostrar_prompt=False, path_errores=None):
    ...
    return caracterizar_variable(df_emociones, df_recortes, modelo_llm, PROMPT_DOMINANCIA, heuristicas_path, ontologia_path,
                                 mostrar_prompt=mostrar_prompt, path_errores=path_errores)
...
def caracterizar_intensidad(df_emociones, df_recortes, modelo_llm, mostrar_prompt=False, path_errores=None):
    ...
    return caracterizar_variable(df_emociones, df_recortes, modelo_llm, PROMPT_INTENSIDAD, heuristicas_path, ontologia_path,
                                 mostrar_prompt=mostrar_prompt, path_errores=path_errores)
...
def caracterizar_fuente(df_emociones, df_recortes, modelo_llm, mostrar_prompt=False, path_errores=None):
    ...
    return caracterizar_variable(df_emociones, df_recortes, modelo_llm, PROMPT_FUENTE, heuristicas_path, ontologia_path,
                                 mostrar_prompt=mostrar_prompt, path_errores=path_errores)
...
def caracterizar_emociones_todas(df_emociones, df_recortes, modelo_llm,
    ...
    resultados["foria"] = caracterizar_foria(
        df_emociones, df_recortes, modelo_llm,
        mostrar_prompt=mostrar_prompt, path_errores=path_errores
    )
    resultados["dominancia"] = caracterizar_dominancia(
        df_emociones, df_recortes, modelo_llm,
        mostrar_prompt=mostrar_prompt, path_errores=path_errores
    )
    resultados["intensidad"] = caracterizar_intensidad(
        df_emociones, df_recortes, modelo_llm,
        mostrar_prompt=mostrar_prompt, path_errores=path_errores
    )
    resultados["fuente"] = caracterizar_fuente(
        df_emociones, df_recortes, modelo_llm,
        mostrar_prompt=mostrar_prompt, path_errores=path_errores
    )

    # --- Renombrar columnas de justificación para evitar choques ---
    rename_map = {
    ...
    for key in ["dominancia", "intensidad", "fuente"]:
        if not resultados[key].empty:
            # dejamos solo las columnas que no estén ya en df_final (además de las claves)
