...
def render(db_path: Path) -> None:
    ...
    st.dataframe(
        analizados[
            ["valor_norm", "n_usos", "funcion", "distribucion",
             "foria_entorno", "acoplamiento", "justificacion"]
        ],
    ...
    funciones = sorted(
        f for f in usos["funcion"].dropna().unique() if str(f).strip()
    )
    filtro = st.multiselect(
        "Filtrar por función del uso", funciones, key=f"hash_ffn_{seleccion}"
    )
