...
class MastodonAdapter(PostSourceAdapter):
    ...
    def fetch_follows(self, handle: str, max_items: int | None = None) -> Iterator[str]:
        ...
        público; en ese caso `_get` levanta y el llamador lo reporta.
        """
        cuenta = self._lookup_account(handle)
        account_id = str(cuenta.get("id") or "")
        if not account_id:
            return
        n = 0
        max_id: str | None = None
        while True:
            ...
            if max_id:
                params["max_id"] = max_id
            pagina = self._get(f"/api/v1/accounts/{account_id}/following", params)
            if not pagina:
                return
            for cuenta_seguida in pagina:
                acct = str(cuenta_seguida.get("acct") or "")
                if not acct:
                    continue
                yield self._qualify_handle(acct)
                n += 1
                if max_items is not None and n >= max_items:
                    return
