...
def _resolve_posicion(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "frase_idx" in out.columns:
        ...
        out["posicion"] = out["recorte_id"].astype(str).str.extract(r"(\d+)$")[0].astype(float)
    return out
...
def _curva_hover(
    ...
    *,
    ...
    modos = (
        df_emo["modo_existencia"] if "modo_existencia" in df_emo.columns else pd.Series(["?"] * n)
    )
    fuentes = (
        df_emo[fuente_col] if fuente_col and fuente_col in df_emo.columns else pd.Series([None] * n)
    )
    ...
    fig.update_layout(
        **_base_layout(
            ...
            yaxis=dict(
                ...
                categoryorder="array",
                categoryarray=list(reversed(emociones)),
            ),
    ...
    *,
    ...
    if orden:
        ...
        counts = counts.sort_values(
            "emocion",
            key=lambda s: s.map(lambda e: pos.get(e, len(pos))),
            ascending=False,
        )
    else:
    ...
    *,
    ...
    fig.update_layout(
        **_base_layout(
            ...
            barmode="stack",
            xaxis=dict(tickangle=-20, gridcolor=BORDER, automargin=True, tickfont=dict(size=10)),
            yaxis=dict(
                gridcolor=BORDER,
    ...
    for codigo in codigos:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers+text",
                name=codigo,
                text=[str(int(x)) for x in xs],
                textposition="top center",
                textfont=dict(size=8, color=TEXT_DIM),
                marker=dict(
                    color=[_color_de(colores, e) for e in ys],
                    size=12,
                    line=dict(color=BORDER, width=1),
                ),
    ...
    fig.update_layout(
        **_base_layout(
            ...
            xaxis=dict(
                ...
                gridcolor=BORDER,
                tickvals=list(range(1, n_bins + 1)),
            ),
            yaxis=dict(
...
def radar_discurso(
    ...
    codigos: list[str],
    *,
    emociones_ref: list[str] | None = None,
) -> go.Figure:
    ...
    fig.update_layout(
        ...
        paper_bgcolor=BG,
        font=dict(family=FONT, color=TEXT_DIM),
...
def scatter_foria_intensidad(
    ...
    *,
    ...
    intens_map = {
        "muy baja": 1,
        "baja": 2,
        "neutra_ambivalente": 3,
        "neutra ambivalente": 3,
        "media": 3,
        "neutra": 3,
        "alta": 4,
        "muy alta": 5,
    ...
    fig = go.Figure()
    actores = df_f[actor_col].unique() if actor_col in df_f.columns else ["(todos)"]
    ...
    for i, actor in enumerate(actores):
        df_act = df_f[df_f[actor_col] == actor] if actor_col in df_f.columns else df_f
...
def timeline_corpus(
    ...
    *,
    ...
    for codigo, df_disc in fechas.groupby("codigo"):
        ...
        enunciador = str(df_disc["enunciador"].iloc[0]) if "enunciador" in df_disc.columns else ""
        if emocion:
            total = len(df_disc)
