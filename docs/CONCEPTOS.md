...
Cada entrada procede, entonces, en tres movimientos que no siempre aparecen separados: la **procedencia** (la tradición de la que retomo el concepto), el **sentido** (qué designa dentro del enfoque de *EmoParse*) y la **operacionalización** (cómo esa noción se convierte en un campo, un valor de un conjunto cerrado o una etapa del *pipeline*). El orden de las secciones sigue, a grandes rasgos, el recorrido de un discurso por el sistema: primero las nociones que fundan todo el enfoque, después la escena enunciativa, los actores y sus marcas, la detección de la emoción, su caracterización, su configuración relacional, y, por último, las categorías propias del discurso nativo digital.

Una aclaración transversal, válida para casi todas las entradas: cuando digo que un campo toma "uno de estos valores", eso no es una recomendación sino una restricción que el sistema impone durante la propia generación del modelo de lenguaje. El conjunto de valores válidos está declarado de antemano, y el modelo no puede devolver nada por fuera de él. Esa clausura tiene una consecuencia teórica, no solo técnica: la grilla categorial con la que el sistema lee permanece explícita, versionada y discutible, en lugar de quedar sedimentada de manera implícita, como ocurre en un clasificador entrenado.
